    ...
    return (
        ...
        <Container className="user-cart-content">
            <h2 className="user-cart-title">My Cart</h2>
                {cartItems.length === 0 ?
                    <Alert className='user-cart-alert' variant="info">Your cart is empty.</Alert>
                    :
                            ...
                            {cartItems.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td className='user-cart-remove-btn'>
                                        <Button  variant="outline-danger" onClick={() => removeFromCart(item)}>Remove</Button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                {cartItems.length > 0 &&
                    <>
                        <h4 className="user-cart-checkout-title">Total: {getTotalPrice()}</h4>
                        <Button onClick={() => navigate('/user-checkout')} className="user-cart-checkout-button" variant="success" size="lg" block>Checkout</Button>
                    </>
                }
            </Container>
        </div>
